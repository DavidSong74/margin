...
const ACCENT = "#7d9b76"; // sage green
    ...
    >
      ...
      >
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
            ...
            <div>
              ...
              <input
                ...
                aria-describedby={email.error ? `${emailId}-error` : undefined}
                value={email.value}
            ...
            <div>
              ...
              <div style={{ position: "relative" }}>
                <input
                  ...
                  aria-describedby={[password.error ? `${passwordId}-error` : "", mode === "signup" ? `${passwordId}-hint` : ""].filter(Boolean).join(" ") || undefined}
              ...
              {mode === "signup" && !password.error && (
                <p id={`${passwordId}-hint`} style={{ ...hintStyle }}>At least 8 characters required.</p>
              )}
              {password.error && <p id={`${passwordId}-error`} role="alert" style={errorStyle}>{password.error}</p>}
            </div>
              ...
              <div>
                ...
                <div style={{ position: "relative" }}>
                  <input
                    ...
                    aria-describedby={confirm.error ? `${confirmId}-error` : undefined}
                    value={confirm.value}
                ...
                {confirm.error && <p id={`${confirmId}-error`} role="alert" style={errorStyle}>{confirm.error}</p>}
              </div>
            )}
            ...
            <button
              ...
              onMouseDown={(e) => { if (!submitting) e.currentTarget.style.transform = "translateY(0)"; }}
            >
              {submitting && <Spinner />}
        ...
        <p style={{ textAlign: "center", marginTop: "22px", marginBottom: 0, color: MUTED, fontSize: "0.875rem" }}>
          {mode === "login" ? (
            <>New here?{" "}
              <button type="button" onClick={() => switchMode("signup")} style={linkButtonStyle}>Create an account</button>
            </>
          ) : (
            <>Already have an account?{" "}
              <button type="button" onClick={() => switchMode("login")} style={linkButtonStyle}>Log in</button>
            </>
          )}
        </p>
      </div>
...
const linkButtonStyle: React.CSSProperties = {
  ...
  textDecorationColor: "rgba(125,155,118,0.3)",
  transition: "color 0.15s",
  fontFamily: "inherit",
};
